...
export function comboVide(): Combo {
  return {
    variant: [{ valeur: AXES[0].options[0], pourcentage: 100 }],
    intensite: [{ valeur: AXES[1].options[0], pourcentage: 100 }],
    nage: [{ valeur: AXES[2].options[0], pourcentage: 100 }],
    pourcentage: 100,
  };
}

// Chaque valeur cochée d'un axe (variant/intensité/nage) porte son propre %
// (réglé par la barre glissable, cf. rendu ci-dessous) — pas de partage à
// parts égales entre valeurs cochées.
export function camembertsAxe(field: "variant" | "intensite" | "nage", combos: Combo[]): CamembertItem[] {
  const sums = new Map<string, number>();
  for (const c of combos) {
    for (const v of c[field]) {
      if (!v.valeur) continue;
      sums.set(v.valeur, (sums.get(v.valeur) ?? 0) + (c.pourcentage * (v.pourcentage || 0)) / 100);
    }
  }
...
function sommeAxe(valeurs: ValeurPourcentage[]) {
  return valeurs.reduce((s, v) => s + (v.pourcentage || 0), 0);
}

// Barre à 100% divisée en un segment par valeur cochée pour cet axe — on
// glisse la poignée entre deux segments pour rééquilibrer leurs deux %, le
// reste ne bouge pas. La somme reste donc toujours exactement 100 par
// construction, sans validation à afficher : plus direct que taper un
// nombre par valeur.
function BarreRepartition({ valeurs, onChange }: { valeurs: ValeurPourcentage[]; onChange: (valeurs: ValeurPourcentage[]) => void }) {
...
export function combosValidesPour(v: SeanceContenu): boolean {
  ...
  const axesOk = v.combos.every(
    (c) => (c.variant.length <= 1 || sommeAxe(c.variant) === 100) && (c.intensite.length <= 1 || sommeAxe(c.intensite) === 100) && (c.nage.length <= 1 || sommeAxe(c.nage) === 100)
  );
  return repartitionsOk && axesOk;
}
...
export function SeanceContenuEditor({
  ...
  // Coche/décoche redistribue tout de suite un % égal entre les valeurs
  // cochées de cet axe (immédiatement utilisable), que l'utilisateur peut
  // ensuite affiner en glissant les poignées de la barre ci-dessous — pas
  // besoin d'ouvrir une répartition séparée par valeur. Au moins une valeur
  // doit rester cochée par axe, donc la dernière ne se décoche pas.
  function repartirEgal(valeurs: string[]): ValeurPourcentage[] {
  ...
  function toggleAxeValeur(index: number, field: "variant" | "intensite" | "nage", valeur: string) {
    onChange((v) => ({
      ...v,
      combos: v.combos.map((c, i) => {
        if (i !== index) return c;
        const present = c[field].some((n) => n.valeur === valeur);
        if (present && c[field].length === 1) return c;
        const valeurs = present ? c[field].filter((n) => n.valeur !== valeur).map((n) => n.valeur) : [...c[field].map((n) => n.valeur), valeur];
        return { ...c, [field]: repartirEgal(valeurs) };
      }),
    }));
  }
  function setAxePourcentages(index: number, field: "variant" | "intensite" | "nage", next: ValeurPourcentage[]) {
    onChange((v) => ({ ...v, combos: v.combos.map((c, i) => (i === index ? { ...c, [field]: next } : c)) }));
  }
  ...
  const camembertsPlan: { titre: string; items: CamembertItem[] }[] =
    value.mode === "auto" && value.combos.length > 1
      ? [
          { titre: "Par nage", items: camembertsAxe("nage", value.combos) },
          { titre: "Par intensité", items: camembertsAxe("intensite", value.combos) },
                  ...
                  {AXES.map((ax) => (
                    <div key={ax.key}>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {ax.options.map((o) => (
                          <Chip key={o} active={combo[ax.key].some((v) => v.valeur === o)} onClick={() => toggleAxeValeur(index, ax.key, o)}>
                            {o}
                          </Chip>
                        ))}
                      </div>
                      {combo[ax.key].length > 1 && <BarreRepartition valeurs={combo[ax.key]} onChange={(next) => setAxePourcentages(index, ax.key, next)} />}
                    </div>
                  ))}
                  {value.combos.length > 1 && (
                    <div className="flex items-center gap-1.5">
