...
export function EntrainementClient({ groupesParPole, plans }: { groupesParPole: Section[]; plans: Plan[] }) {
  ...
  const [selectedGroupeId, setSelectedGroupeId] = useState<string | null>(toutGroupes[0]?.id ?? null);
  const selected = toutGroupes.find((g) => g.id === selectedGroupeId) ?? null;
  const plansDuGroupe = selected ? plans.filter((p) => p.groupes.some((g) => g.id === selected.id)) : [];
  ...
  function planPourCellule(themeNom: string, semaine: Date) {
    return plansDuGroupe.find((p) => p.theme === themeNom && new Date(p.dateDebut) <= semaine && semaine <= new Date(p.dateFin));
  }

  function ouvrirNouveau(themeNom: string, semaine: Date) {
    if (!selected) return;
    const dernierePourTheme = plansDuGroupe.filter((p) => p.theme === themeNom).sort((a, b) => a.dateFin.localeCompare(b.dateFin)).at(-1);
    const debutParDefaut = dernierePourTheme ? ajouterJours(new Date(dernierePourTheme.dateFin), 1) : semaine;
    setForm(formulaireVide(themeNom, debutParDefaut, selected.id));
    setModal({ mode: "new" });
  }
          ...
          <div key={section.pole} className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {section.groupes.map((groupe) => {
                const actif = groupe.id === selectedGroupeId;
                return (
                  <button
                    key={groupe.id}
            ...
            <div style={{ minWidth: LARGEUR_COLONNE + SEMAINES_AFFICHEES * 56 }}>
              ...
              {OBJECTIFS.map((objectif) => (
                <div key={objectif.nom} className="flex items-stretch" style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  {semaines.map((s, i) => {
                    const plan = planPourCellule(objectif.nom, s);
                    return (
                      <div
                        key={i}
                        className="shrink-0 flex items-center justify-center"
                        style={{ width: 56, padding: "6px 3px", cursor: "pointer" }}
                        onClick={() => (plan ? ouvrirEdition(plan) : ouvrirNouveau(objectif.nom, s))}
